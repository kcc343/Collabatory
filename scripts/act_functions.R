...
# Access each of the actor/actress csv files and returns a box plot for
# specified data (budget or revenue)
...
act_analysis <- function(name, type) {
  ...
  p <- ggplot(
    ...
    ggtitle(
      paste0(
        toTitleCase(name), "'s Movie ", toTitleCase(type), " Boxplot"
      )
    ) +
